...
test_syntax_validation() {
    ...
    # Define required variables before sourcing core.sh
    export IS_CADDY_DIR="${IS_CADDY_DIR:-/etc/caddy}"
    export IS_NGINX_DIR="${IS_NGINX_DIR:-/etc/nginx}"
    export AUTHOR="${AUTHOR:-WangYan-Good}"
...
echo DEBUG_SYN002_DONE >> /tmp/test_debug.txt
        test_result=false
    fi
...
test_variable_expansion() {
    ...
    local ws_stream=$(generate_stream_settings ws "tls" "$test_host" "$test_path" 2>/dev/null)
...
test_json_parsing() {
    ...
    IS_STREAM_SETTINGS_JSON=$(generate_stream_settings tcp \
        "reality" \
        "${IS_SERVERNAME}:443" \
        "[\"${IS_SERVERNAME}\",\"\"]" \
        "$IS_PUBLIC_KEY" \
...
generate_summary() {
    ...
    if [[ $total_count -gt 0 ]]; then
        if command -v bc &> /dev/null; then
            ...
            # Alternative calculation without bc
