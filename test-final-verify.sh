...
echo "=== V2Ray Phase 9 - Final Verification ==="
echo ""
...
export IS_DONT_GET_IP=1

# 设置必要的变量以避免错误
export IS_SH_DIR="/etc/v2ray/sh"
export IS_CORE_NAME="v2ray"
export IS_CADDY=""
export IS_NGINX=""
export IS_CADDY_CONF=""
export IS_NGINX_CONF=""

# 加载依赖的模块
if [ -f src/init.sh ]; then
    source src/init.sh
else
    # 临时定义必要的函数（模拟 init.sh）
    IS_ERR=$'\e[41m错误!\e[0m'
    err() {
        echo -e "\n$IS_ERR $*\n"
