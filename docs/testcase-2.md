...
|---------|----------|----------|--------|------|
| TC-01 | ✅ 通过 | 2026-04-07 13:15 | Qwen | XHTTP JSON 生成成功 (gen 命令) |
| TC-02 | ⏭️ 跳过 | - | - | 需要有效域名+Nginx证书，测试环境无可用域名 |
| TC-03 | ⏭️ 跳过 | - | - | 同上 |
| TC-04 | ✅ 通过 | 2026-04-07 13:15 | Qwen | Trojan-XHTTP-TLS 已在 protocol_list 中注册 |
| TC-05 | ⏭️ 跳过 | - | - | 需要完整 TLS 配置 |
| TC-06 | ⏭️ 跳过 | - | - | 需要完整配置 |
| TC-07 | ⏭️ 跳过 | - | - | 跳过 |
| TC-08 | ✅ 通过 | 2026-04-07 13:20 | Qwen | JSON 结构正确: network=xhttp, xhttpSettings={path, host, mode:"auto"} |
| TC-09 | ⏭️ 跳过 | - | - | 需要 Nginx+Certbot 环境 |
| TC-10 | ✅ 通过 | 2026-04-07 13:25 | Qwen | XHTTP 端口 50064 正常监听，服务无错误 |

**服务端通过率**: 4/10 = 40% (跳过的均为需要 TLS/域名环境)

### 客户端端到端测试 (192.168.1.110 → bak.proxy.yourdie.com)

| 用例 ID | 测试内容 | 测试结果 | 执行时间 | 备注 |
|---------|----------|----------|----------|------|
| E2E-01 | XHTTP 代理访问 httpbin.org | ✅ 通过 | 2026-04-07 13:30 | HTTP 和 SOCKS5 代理均正常 |
| E2E-02 | 出口 IP 验证 | ✅ 通过 | 2026-04-07 13:30 | 出口IP=107.174.218.158 (服务器IP) |
| E2E-03 | Google via XHTTP SOCKS5 | ✅ 通过 | 2026-04-07 13:30 | HTTP/2 200 |
| E2E-04 | YouTube via XHTTP SOCKS5 | ✅ 通过 | 2026-04-07 13:30 | HTTP/2 200，视频网站可访问 |
| E2E-05 | REALITY + XHTTP 协议共存 | ✅ 通过 | 2026-04-07 13:30 | 两个客户端实例同时运行，互不干扰 |
...
| # | 问题 | 修复状态 |
|---|------|----------|
| 1 | `*xhttp*` 被 `*http*` 优先匹配，导致 mode 错误为 `stream-one` | ✅ 已修复: 将 `*xhttp*` 移到 `*http*` 之前 |
| 2 | `vxhttp | txhttp` 快捷输入未覆盖 `vless-xhttp-tls` 全称 | ✅ 通过 `*` 回退匹配 protocol_list 解决 |
